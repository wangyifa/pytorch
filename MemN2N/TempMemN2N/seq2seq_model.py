...
class MemN2N_Seq2Seq(nn.Module):
    def __init__(self, corpus_dict_size, mem_size, emb_size, hidden_size, batch_size, 
                hops, max_length, dropout, teach_forcing, memn2n_type, **kw):
        super(MemN2N_Seq2Seq, self).__init__()
        self.corpus_dict_size = corpus_dict_size
        ...
        if self.memn2n_type == 0:
            print "Adjacent:"
            self.encoder = TempAjcnMemN2N(self.mem_size, self.hidden_size, self.emb_size, self.batch_size, self.hops)
        else:
            print "Layer-wise:"
            self.encoder = TempLayerMemN2N(self.mem_size, self.hidden_size, self.emb_size, self.batch_size, self.hops)
        self.decoder = Decoder(self.corpus_dict_size, self.hidden_size, self.emb_size)

        # embedding层
        self.embedding = nn.Embedding(self.corpus_dict_size,self.emb_size)

        # 初始化embedding层参数
        if kw.has_key('emb_matrix'):
            print "get emb_matrix"
            self.embedding.weight = nn.Parameter(kw['emb_matrix'])
            self.using_w2v = True
            print ""
    ...
    def forward(self, post_tensor_batch, reply_tensor_batch, mems_tensor_batch,ini_idx):
        ...
        for i in range(self.mem_size):
            ...
            input = self.embedding(Variable(mems_tensor_batch[i]).cuda())
            inputs[i] = self.dropout(input)

        # questions: batch_size*max_length*emb_size
        questions = self.embedding(Variable(post_tensor_batch).cuda())
