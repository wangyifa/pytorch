...
def readingCorpus(post_file, reply_file, mem_size):
    ...
    for post_line,reply_line in zip(PostFile,ReplyFile):
        post_line = post_line.strip('\n').strip()
        reply_line = reply_line.strip('\n').strip()
        list_post.append(post_line)
        list_reply.append(reply_line)
        list_mem.append([post_line,reply_line])
...
def filteringPair(post_word2index,reply_word2index,mem_word2index,corpus_pair,unk_char,ini_char):
    filtering_pair = []
    post_sentence = corpus_pair[0]
    reply_sentence = corpus_pair[1]
    mem = corpus_pair[2]
    ...
    tmp_mem = []
    for mem_sentence in mem:
        mem_sentence = filteringSenten(mem_word2index,mem_sentence,unk_char,ini_char)
        tmp_mem.append(mem_sentence)
...
def filteringPairs(post_word2index,reply_word2index,mem_word2index,corpus_pairs,unk_char,ini_char):
    filtering_pairs = []
    for corpus_pair in corpus_pairs:
        corpus_pair = filteringPair(post_word2index,reply_word2index,mem_word2index,corpus_pair,unk_char,ini_char)
        filtering_pairs.append(corpus_pair)
    return filtering_pairs
#####################################################

# 读取语料，构建词表的主函数
def readingData(post_w2v, reply_w2v, all_w2v, post_file, reply_file, ini_char, unk_char, mem_size, max_length):
    print "loading corpus..."
    corpus_pairs = readingCorpus(post_file, reply_file, mem_size)
    post_ctable,reply_ctable,mem_ctable = readingW2v(post_w2v,reply_w2v,all_w2v,ini_char,unk_char)

    # 后处理：替换语料中不在w2v词表的词和padding操作
    corpus_pairs = filteringPairs(post_ctable.getWord2Index(),reply_ctable.getWord2Index(),mem_ctable.getWord2Index(),
    ...
    return post_ctable,reply_ctable,mem_ctable,corpus_pairs

# 读取w2v文件，构建emb矩阵
def readingW2v(post_w2v, reply_w2v, mem_w2v, ini_char, unk_char):
    print "loading word2vec..."
    post_ctable = W2vCharacterTable(post_w2v,ini_char,unk_char)
    reply_ctable = W2vCharacterTable(reply_w2v,ini_char,unk_char)
    mem_ctable = W2vCharacterTable(mem_w2v,ini_char,unk_char)

    return post_ctable,reply_ctable,mem_ctable
...
def tensorsFromMem(mem_word2index,mem):
    list_mem_tensor = []
    for m in mem:
        mem_tensor = tensorFromSentence(mem_word2index,m)
        list_mem_tensor.append(mem_tensor)
    return list_mem_tensor

def tensorsFromPair(post_word2index,reply_word2index,mem_word2index,pair):
...
def getTensorsPairsBatch(post_word2index,reply_word2index,mem_word2index,pairs_batches,mem_size):
    for batch in pairs_batches:
        ...
        for pair in batch:
            tensor_q,tensor_r,list_tensor_m = tensorsFromPair(post_word2index,reply_word2index,mem_word2index,pair)
...
def getTensorsSentenMemBatch(post_word2index,mem_word2index,sentence_batches,mem_batches,mem_size):
    for sentence_batch,mem_batch in zip(sentence_batches,mem_batches):
        list_s = []
        list_m = [[] for i in range(mem_size)]
        mems_tensor_batch = []
        for sentence in sentence_batch:
            tensor_sentence = tensorFromSentence(post_word2index,sentence)
            list_s.append(tensor_sentence.t())

        for mem in mem_batch:
            list_tensor_m = tensorsFromMem(mem_word2index,mem)
            for idx,tensor_m in enumerate(list_tensor_m):
                list_m[idx].append(tensor_m.t())
...
if __name__ == '__main__':
    post_w2v = "/users3/yfwang/rg_data/w2v/ecm_filt_tab_q_500e.w2v"
    reply_w2v = "/users3/yfwang/rg_data/w2v/ecm_filt_tab_a_500e.w2v"
    all_w2v = "/users3/yfwang/rg_data/w2v/ecm_filt_tab_all_500e.w2v"

    post_file = "/users3/yfwang/rg_data/corpus/ecm_filt_tab1000.q"
    reply_file = "/users3/yfwang/rg_data/corpus/ecm_filt_tab1000.a"
    
    post_ctable,reply_ctable,mem_ctable,corpus_pairs = readingData(post_w2v, reply_w2v, all_w2v, post_file, reply_file, ini_char='</i>', 
    ...
    mem_word2index = mem_ctable.getWord2Index()
    mem_index2word = mem_ctable.getIndex2Word()
    mem_emb_matrix = mem_ctable.getEmbMatrix()

    unk_index = post_word2index['<unk>']
    unk_emb = post_emb_matrix[unk_index]

    pairs_batches,num_batches = buildingPairsBatch(corpus_pairs,batch_size=32,shuffle=True)
    print " num_batches:",num_batches

    pairs_batch = pairs_batches[0]
    print pairs_batch[0][0]
    print pairs_batch[0][1]
    for line in pairs_batch[0][2]:
        print line
