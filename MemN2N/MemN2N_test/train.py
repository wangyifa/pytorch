...
# 多轮训练函数
def train_model(post_word2index,mem_word2index,reply_word2index,ini_idx,corpus_pairs,model,model_optimizer,criterion,epochs,
    ...
    for ei in range(epochs):
        ...
        idx_batch = 0
        for post_vari_batch,reply_vari_batch,mems_vari_batch in getTensorsPairsBatch(post_word2index,reply_word2index,mem_word2index,pairs_batches,mem_size):
...
if __name__ == '__main__':
    ini_char = '</i>'
    unk_char = '<unk>'
    post_ctable,reply_ctable,mem_ctable,corpus_pairs = readingData(opts.w2v_path+opts.post_w2v,opts.w2v_path+opts.reply_w2v,opts.w2v_path+opts.mem_w2v, 
    ...
    if opts.using_w2v:
        memn2n_seq2seq = MemN2N_Seq2Seq(post_ctable.getDictSize(),mem_ctable.getDictSize(),reply_ctable.getDictSize(),opts.mem_size,
                                        post_ctable.getEmbSize(),opts.hidden_size,opts.batch_size,opts.hops,opts.max_length,opts.dropout,
                                        opts.teach_forcing,opts.memn2n_type,post_emb_matrix=post_ctable.getEmbMatrix(),
                                        mem_emb_matrix=mem_ctable.getEmbMatrix(),dec_emb_matrix=reply_ctable.getEmbMatrix()).cuda()
    else:
        memn2n_seq2seq = MemN2N_Seq2Seq(post_ctable.getDictSize(),mem_ctable.getDictSize(),reply_ctable.getDictSize(),opts.mem_size,
                                        corpus_ctable.getEmbSize(),opts.hidden_size,opts.batch_size,opts.hops,opts.max_length,opts.dropout,
                                        opts.teach_forcing,opts.memn2n_type).cuda()
    ...
    train_model(post_ctable.getWord2Index(),mem_ctable.getWord2Index(),reply_word2index,ini_idx,corpus_pairs,memn2n_seq2seq,
                model_optimizer,criterion,opts.epochs,opts.batch_size,opts.max_length,opts.mem_size,opts.print_every,opts.save_model,opts.shuffle)
    print ""
