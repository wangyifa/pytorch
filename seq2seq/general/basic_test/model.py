...
class Seq2Seq(nn.Module):
    def __init__(self, encoder_emb_matrix, decoder_emb_matrix, encoder_dict_size, decoder_dict_size, 
        ...
        self.decoder = Decoder(self.decoder_dict_size, self.hidden_size, self.emb_size, self.num_layers, self.dropout)
    ...
    def forward(self, post_tensor_batch, reply_tensor_batch, ini_idx):
        ...
        for post_tensor in post_tensor_batch:
            post_embedded = self.enc_embedding(Variable(post_tensor).cuda())
            post_embeddeds.append(post_embedded)

        # encoder_embedded: batch_size*max_length*emb_size
        encoder_embedded = torch.stack(tuple(post_embeddeds),1)
        encoder_hidden = self.encoder(encoder_embedded,encoder_hidden)
