...
def predict_Sentences(post_word2index,reply_index2word,unk_char,model,
                    post_file_path,print_every,batch_size,max_length):
    model.eval()
    ...
    for sentence in post_sentences:
        sentence = filteringSenten(post_word2index,sentence,unk_char)
    ...
    for post_batch in getTensorsSentenBatch(post_word2index,sentence_batches):
        predict_batch = model.predict(post_batch,reply_index2word,sep_char=' ')
...
if __name__ == '__main__':
    unk_char = '<unk>'
    post_ctable,reply_ctable = readingW2v(opts.w2v_path+opts.post_w2v, opts.w2v_path+opts.reply_w2v, unk_char)
    ...
    post_sentences,decoded_sentences = predict_Sentences(post_ctable.getWord2Index(),reply_ctable.getIndex2Word(),post_ctable.getUnkChar(),
                                                        seq2seq_model,post_file_path,opts.print_every,opts.batch_size,opts.max_length)

    pred_file = open("pred_result_lts",'w')
    pred_a_file = open("pred_answer_lts",'w')
