...
class W2vCharacterTable(object):
    def __init__(self, w2v_path, unk_char='<unk>'):
...
#####################################################
# 未登录词替换成unk_char
def filteringSenten(word2index,sentence,unk_char):
    ListSenten = sentence.split('\t')
    filteringListSenten = []
    ...
    if filteringListSenten == []:
        filteringListSenten = [unk_char]
    return '\t'.join(filteringListSenten)

def filteringPair(post_word2index,reply_word2index,corpus_pair,unk_char):
    filtering_pair = []
    post_sentence = corpus_pair[0]
    reply_sentence = corpus_pair[1]
    post_sentence = filteringSenten(post_word2index,post_sentence,unk_char)
    reply_sentence = filteringSenten(reply_word2index,reply_sentence,unk_char)
    filtering_pair.append(post_sentence)
    filtering_pair.append(reply_sentence)
    return filtering_pair

def filteringPairs(post_word2index,reply_word2index,corpus_pairs,unk_char):
    filtering_pairs = []
    for corpus_pair in corpus_pairs:
        corpus_pair = filteringPair(post_word2index,reply_word2index,corpus_pair,unk_char)
        filtering_pairs.append(corpus_pair)
    return filtering_pairs
#####################################################

# 读取语料，构建词表的主函数
def readingData(post_w2v, reply_w2v, post_file, reply_file, unk_char, max_length):
    print "loading corpus..."
    corpus_pairs = readingCorpus(post_file, reply_file)

    post_ctable,reply_ctable = readingW2v(post_w2v,reply_w2v,unk_char,)
    # 后处理：去掉语料中不在w2v词表的词和padding操作
    corpus_pairs = filteringPairs(post_ctable.getWord2Index(),reply_ctable.getWord2Index(),
                                corpus_pairs,post_ctable.getUnkChar())
...
# 读取w2v文件，构建emb矩阵
def readingW2v(post_w2v, reply_w2v, unk_char):
    print "loading word2vec..."
    post_ctable = W2vCharacterTable(post_w2v,unk_char)
    reply_ctable = W2vCharacterTable(reply_w2v,unk_char)
...
if __name__ == '__main__':
    ...
    post_ctable,reply_ctable,corpus_pairs = readingData(post_w2v, reply_w2v, post_file, reply_file,
