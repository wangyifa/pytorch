...
'''单个batch的训练函数'''
def train_batch(post_tensor_batch,reply_tensor_batch,model,model_optimizer,criterion,ini_idx):
    loss = 0
    model_optimizer.zero_grad()

    list_pred = model(post_tensor_batch,reply_tensor_batch,ini_idx)
...
# 多轮训练函数
def train_model(post_word2index,reply_word2index,post_index2word,reply_index2word,ini_idx,corpus_pairs,model,
    ...
    for ei in range(epochs):
        ...
        for post_tensor_batch,reply_tensor_batch in getTensorsPairsBatch(post_word2index,reply_word2index,pairs_batches):
            loss = train_batch(post_tensor_batch,reply_tensor_batch,model,model_optimizer,criterion,ini_idx)
            epoch_loss += loss
            every_loss += loss
...
if __name__ == '__main__':
    ...
    post_ctable,reply_ctable,corpus_pairs = readingData(opts.w2v_path+opts.post_w2v, opts.w2v_path+opts.reply_w2v, 
                                                        ...
                                                        ini_char, unk_char, opts.max_length)
    ...
    train_model(post_ctable.getWord2Index(),reply_word2index,post_ctable.getIndex2Word(),reply_ctable.getIndex2Word(),
                ini_idx,corpus_pairs,seq2seq_model,model_optimizer,criterion,opts.epochs,opts.batch_size,
                opts.max_length,opts.print_every,opts.save_model,opts.shuffle)
    print ""
